...
public class PlayerController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        slideLimit = movement.controller.slopeLimit - 0.1f;
    ...
    void Update()
    {
        ...
        //HeadBob
        if (movement.controller.velocity.x == 0 && movement.controller.velocity.y == 0) {
            HeadBob(idleCounter, 0.025f, 0.025f);
            idleCounter += Time.deltaTime;
            weaponParent.localPosition = Vector3.Lerp(weaponParent.localPosition, targetWeaponBobPosition, Time.deltaTime * 2f);
        }
        else if(playerInput.run && status == Status.moving) {
    ...
    bool canSlide(){
        if (!movement.grounded)
            return false;
        if (playerInput.input.magnitude <= 0.02f || !playerInput.run)
            return false;
        if (slideTime > 0 || status == Status.sliding)
            return false;
